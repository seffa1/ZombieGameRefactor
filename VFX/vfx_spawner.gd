...
@export_group('Has Wrapper Node')
@export var has_wrapper_node: bool = false
...
func spawn_item(rotation: float):
	...
	if !has_wrapper_node:
		...
		# Boss limbs have a wrapper Node2D around the rigid body to control animations
		instance.handle_imuplse(impulse_vector)
		instance.handle_torque(item_ejection_torque + randf_range(-item_ejection_torque_variance, item_ejection_torque_variance))
